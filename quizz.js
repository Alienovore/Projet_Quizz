...
function chooseTheme() {
    let theme = document.getElementsByName("options").value;
    console.log(theme);
}
...
function loadQuestion() {
    ...
    if (index_question < nb_question) {
        ...
        document.getElementById("index_question").innerHTML = "Question: " + (index_question + 1);
        fetch(`http://localhost:8080/internet.json`)
            ...
            .then(data => {
                ...
                answers.push({ "reponse": data.results[index_question].reponse_correcte, "value": "correct" });
                ...
                for (let i = 0; i < answers.length; i++) {
                    ...
                    if (answers[i].value == "correct") {
                        document.getElementById("btn" + i).setAttribute("onclick", "correct()");
                        right_ans = answers[i].reponse;
                    }
                    else {
                        document.getElementById("btn" + i).setAttribute("onclick", "uncorrect()");
                    }
                }
                return right_ans;
            });
